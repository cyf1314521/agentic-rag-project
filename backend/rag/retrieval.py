...
class Retriever:
    ...
    def __init__(
        ...
        llm: Optional[object] = None,  # 大语言模型实例；为 None 时无法使用 HyDE
    ...
    def retrieve(
        ...
        if self.cache:  # 若构造时启用了缓存
            cached = self.cache.get(query)  # 用 query 的语义向量查找相近历史 query 的缓存结果
            if cached is not None:  # 命中缓存
                logger.debug(f"Cache hit for query: {query[:50]}...")  # 调试日志，只打印 query 前 50 字符
                return cached  # 跳过后续检索，直接返回缓存的 Document 列表
        ...
        if self.cache:  # 若启用缓存，将本次结果写入缓存供相似 query 复用
    ...
    def _rerank(self, query: str, docs: list[Document], top_k: int) -> list[Document]:
        ...
        scores = self.reranker.predict(pairs)  # 得到每条候选的相关性分数（越高越相关）
        # 将文档与分数 zip 后按分数降序排序
        ranked = sorted(zip(docs, scores), key=lambda x: x[1], reverse=True)
        # 取前 top_k 个文档，丢弃分数（_ 表示忽略）
    ...
    def _hyde(self, query: str) -> str:
        ...
        try:
            ...
            # LangChain AIMessage 有 .content；其他实现则转字符串
